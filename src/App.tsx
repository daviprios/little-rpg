...
export default function App() {
	...
	function playerActions({
		...
		if (type === 'ATTACK') {
			...
			const fireballDamage = Math.trunc(
				Math.random() * player.randomDamage + player.fireballBaseDamage
			)
	...
	function dragonAction() {
		...
		if (type === 'ATTACK') {
			...
			const fireballDamage = Math.trunc(
				Math.random() * dragon.randomDamage + dragon.fireballBaseDamage
			)
	...
	return (
		<div className='h-full w-full bg-slate-900 text-slate-100'>
			<div className='h-full w-full flex justify-center items-center'>
				<div className='border-2 border-slate-600 rounded-md w-80 h-80 p-2 flex flex-col justify-between gap-2'>
					<div>
						<HealthBar {...player} />
					...
					<div className='grid grid-cols-2 grid-rows-2 gap-2'>
						...
						<button
							className='border-2 p-2 w-full rounded-sm disabled:bg-slate-500 disabled:active:bg-slate-500 disabled:cursor-not-allowed hover:bg-slate-700 active:bg-slate-600'
							disabled={
								!isPlayerTurn || isGameEnd || player.fireballCount === 0
							}
